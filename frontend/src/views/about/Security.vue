...
<template>
  <div class="min-h-screen bg-[var(--color-surface)]">
    ...
    <div class="sticky top-0 left-0 right-0 h-[50px] bg-[var(--color-surface)] shadow-md flex items-center px-4 z-[1000]">
      <span class="text-[var(--color-primary)] text-[15px] cursor-pointer mr-4" @click="router.back()">{{ t('about.back') }}</span>
      <h2 class="flex-1 text-center text-lg font-medium text-[var(--c-text-1)] m-0">{{ t('about.securityTitle') }}</h2>
    ...
    <div class="max-w-2xl mx-auto px-4 py-6">
      <div class="bg-[var(--color-surface)] rounded-xl p-4 shadow-sm">
        <div v-if="isNonChinese" class="bg-amber-50 text-amber-800 border border-amber-400 rounded-md px-4 py-3 mb-4 text-sm leading-relaxed">
        ...
        <div class="text-sm leading-relaxed text-[var(--c-text-2)] [&_p]:my-3 [&_strong]:text-base [&_strong]:font-bold [&_strong]:text-[var(--c-text-1)]">
          <p class="text-center">《广东二师助手安全技术规格说明》</p>
          <p class="text-center">更新日期：2026年3月24日</p>
          ...
          <div class="mb-6">
            ...
            <p class="indent-7">
              （2）HTTPDNS：HTTPDNS是阿里云面向移动开发者推出的一款域名解析产品，具有域名防劫持、精准调度的特性。
            ...
            <p class="indent-7">
              （3）校园卡充值安全保障：为保障用户的资金安全，校园卡充值接口采用了多重安全措施：全站 HTTPS 加密传输、JWT 身份认证、充值前二次密码验证、接口速率限制（每分钟最多3次请求）、客户端 HTTPS 证书固定（Certificate Pinning）、HMAC-SHA256 请求签名以及设备绑定校验。
              HMAC 请求签名确保充值请求参数在传输和处理过程中未被篡改，即使应用运行环境被 Hook 工具（如 Xposed/Frida）注入，也能有效防止关键参数（如充值金额）被恶意修改。
              设备绑定校验要求充值操作必须在用户首次登录时绑定的设备上进行，防止令牌被导出至其他设备使用。
              充值过程中，服务端还会校验支付平台返回的用户信息和充值金额是否与请求一致，防止会话劫持或金额篡改。充值相关的敏感字段（如密码）在日志记录时会被自动脱敏处理。</p>
          </div>
          <div class="mb-6">
            ...
            <p class="indent-7">
              （2）基于对称加密算法的关键用户数据加密存储：在应用数据库和缓存中保存的关键用户数据都采用了 AES 对称加密算法进行加密存储，加密密钥通过环境变量注入，不保存在代码仓库中。生产环境强制启用字段加密，未配置时拒绝启动。</p>
            <p class="indent-7">
          ...
          <div class="mb-6">
            ...
            <p class="indent-7">HMAC算法：密钥散列消息认证码（英语：Keyed-hash message authentication code），又称散列消息认证码（Hash-based message
              authentication
            ...
            <p class="indent-7">OAuth 2.0是OAuth协议的下一版本，但不向下兼容OAuth 1.0。OAuth
