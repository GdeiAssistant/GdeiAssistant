  ...
  >
    ...
    <div class="community-desktop-switch flex bg-[var(--c-card)] mt-2 h-11">
      <a
        ...
        :class="activeArea === 0 ? 'bg-[var(--c-dating)] text-white' : 'text-[var(--c-text-2)]'"
        @click.prevent="switchArea(0)"
      >{{ copy.femaleTab }}</a>
      <a
        href="javascript:;"
        class="flex-1 text-center leading-[44px] text-xl transition-colors duration-300"
        :class="activeArea === 1 ? 'bg-[var(--c-dating)] text-white' : 'text-[var(--c-text-2)]'"
    ...
    <div class="community-desktop-card-grid p-4">
      ...
      >
        ...
        <h2 class="m-0 mb-1.5 text-xl font-bold text-[var(--c-dating)] leading-tight">{{ item.name }}</h2>
