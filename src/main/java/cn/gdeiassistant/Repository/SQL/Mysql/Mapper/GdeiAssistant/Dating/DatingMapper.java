package cn.gdeiassistant.Repository.SQL.Mysql.Mapper.GdeiAssistant.Dating;
...
public interface DatingMapper {
    ...
    List<DatingProfile> selectDatingProfilePage(@Param("start") Integer start
    ...
    DatingProfile selectDatingProfileById(Integer profileId);

    @Select("select * from dating_profile where username=#{username}")
    @ResultMap("DatingProfile")
    List<DatingProfile> selectDatingProfileByUsername(String username);
    ...
    void updateDatingProfile(DatingProfile datingProfile);

    @Update("update dating_profile set state=#{state} where profile_id=#{profileId}")
    void updateDatingProfileState(@Param("profileId") Integer profileId, @Param("state") Integer state);
    ...
    Integer insertDatingProfile(DatingProfile datingProfile);
    ...
    DatingPick selectDatingPickById(Integer pickId);
    ...
    DatingPick selectDatingPick(@Param("profileId") Integer profileId, @Param("username") String username);
    ...
    Integer insertDatingPick(DatingPick datingPick);

    @Update("update dating_pick set state=#{state} where pick_id=#{pickId}")
    void updateDatingPickState(@Param("pickId") Integer pickId, @Param("state") Integer state);
    ...
    List<DatingMessage> selectUserDatingMessagePage(@Param("username") String username
            , @Param("start") Integer start, @Param("size") Integer size);

    @Select("select count(message_id) from dating_message where username=#{username} and state=0")
    @ResultType(Integer.class)
    Integer selectUserUnReadDatingMessageCount(String username);

    @Insert("insert into dating_message (username,pick_id,type,state) values(#{username},#{datingPick.pickId},#{type},#{state})")
    void insertDatingMessage(DatingMessage datingMessage);

    @Update("update dating_message set state=#{state} where message_id=#{messageId}")
    void updateDatingMessageState(@Param("messageId") Integer messageId, @Param("state") Integer state);
}
