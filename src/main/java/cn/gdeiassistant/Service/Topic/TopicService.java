package cn.gdeiassistant.Service.Socialising.Topic;
...
public class TopicService {
    ...
    public String DownloadTopicItemPicture(int id, int index) {
        return aliyunOssUtils.GeneratePresignedUrl("gdeiassistant-userdata", "topic/" + id + "_" + index + ".jpg"
                , 90, TimeUnit.MINUTES);
    }
    ...
    public void UploadTopicItemPicture(int id, int index, InputStream inputStream) {
        aliyunOssUtils.UploadOSSObject("gdeiassistant-userdata", "topic/" + id + "_" + index + ".jpg"
                , inputStream);
        try {
            if (inputStream != null) {
                inputStream.close();
