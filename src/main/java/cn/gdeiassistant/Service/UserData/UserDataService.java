...
public class UserDataService {
    ...
    public void ExportUserData(String sessionId) throws IOException {
        ...
        try {
            ...
            //下载用户头像
            InputStream avatar = aliyunOssUtils.DownloadOSSObject("gdeiassistant-userdata"
                    , "avatar/" + user.getUsername() + ".jpg");
            if (avatar != null) {
                userDataMap.put("avatar.jpg", avatar);
            }
            //下载用户高清头像
            InputStream avatarHD = aliyunOssUtils.DownloadOSSObject("gdeiassistant-userdata"
            ...
            if (ershouItemList != null && !ershouItemList.isEmpty()) {
                for (ErshouItem ershouItem : ershouItemList) {
                    //下载二手交易图片
                    for (int i = 0; i <= 3; i++) {
                        InputStream image = aliyunOssUtils.DownloadOSSObject("gdeiassistant-userdata", "ershou/"
            ...
            if (lostAndFoundItemList != null && !lostAndFoundItemList.isEmpty()) {
                for (LostAndFoundItem lostAndFoundItem : lostAndFoundItemList) {
                    //下载失物招领图片
                    for (int i = 0; i <= 3; i++) {
                        InputStream image = aliyunOssUtils.DownloadOSSObject("gdeiassistant-userdata", "lostandfound/"
            ...
            if (secretList != null && !secretList.isEmpty()) {
                for (Secret secret : secretList) {
                    ...
                    if (secret.getType().equals(1)) {
                        //下载语音信息
                        InputStream voice = aliyunOssUtils.DownloadOSSObject("gdeiassistant-userdata", "secret/voice/" + secret.getId() + ".mp3");
                        if (voice != null) {
            ...
            if (photographList != null && !photographList.isEmpty()) {
                for (Photograph photograph : photographList) {
                    ...
                    for (int i = 1; i <= photograph.getCount(); i++) {
                        InputStream image = aliyunOssUtils.DownloadOSSObject("gdeiassistant-userdata", "photograph/"
            ...
            aliyunOssUtils.UploadOSSObject("gdeiassistant-userdata", "export/" + uuid + ".zip"
    ...
    public String DownloadUserData(String sessionId) {
        ...
        if (StringUtils.isNotBlank(token)) {
            aliyunOssUtils.GeneratePresignedUrl("gdeiassistant-userdata", "export/" + token + ".zip"
                    , 90, TimeUnit.MINUTES);
        }
        return url;
    }
