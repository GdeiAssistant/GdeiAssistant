...
import cn.gdeiassistant.common.pojo.Entity.EncryptedData;
...
import cn.gdeiassistant.common.tools.Utils.AESUtils;
...
public class SecurityAspect {
    ...
    @Pointcut("@annotation(cn.gdeiassistant.common.annotation.RequireSecurity)")
    ...
    public DataJsonResult SecurityAction(ProceedingJoinPoint proceedingJoinPoint) throws Throwable {
        ...
        if (requestParams != null && requestValidation != null && requestSecurity != null) {
            ...
            if (new String(RSAUtils.decryptBytesWithPublicKey(Base64.getDecoder().decode(requestSecurity.getClientRSAPublicKey())
                    , Base64.getDecoder().decode(requestSecurity.getClientRSASignature())))
                    .equals(hash)) {
                //数字签名校验通过，解密客户端AES密钥
                String clientAESKey = Base64.getEncoder().encodeToString(RSAUtils.decryptBytesWithPrivateKey(RSAUtils.getPrivateKey()
                ...
                if (dataJsonResult.getData() != null) {
                    ...
                    String signature = Base64.getEncoder().encodeToString(RSAUtils.encryptBytesWithPrivateKey(RSAUtils.getPrivateKey()
