...
public class WechatAccountUtils {
    ...
    private static final Logger logger = LoggerFactory.getLogger(WechatAccountUtils.class);
    ...
    private static void LoadSchoolWechatAccountData() {
        Resource resource = new ClassPathResource("account.xml");
        SAXReader saxReader = new SAXReader();
        Document document = null;
        try {
            document = saxReader.read(resource.getInputStream());
        } catch (Exception e) {
            logger.error("加载校园公众号配置 account.xml 失败", e);
        }
