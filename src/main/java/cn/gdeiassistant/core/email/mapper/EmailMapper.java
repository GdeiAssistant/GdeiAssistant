...
public interface EmailMapper {
    ...
    @Insert("insert into email (username,email,gmt_create,gmt_modified) values(#{username},#{email,typeHandler=cn.gdeiassistant.common.typehandler.MybatisEncryptionTypeHandler,jdbcType=VARCHAR},now(),now())")
    void insertEmail(@Param("username") String username, @Param("email") String email);

    @Update("update email set email=#{email,typeHandler=cn.gdeiassistant.common.typehandler.MybatisEncryptionTypeHandler,jdbcType=VARCHAR},gmt_modified=now() where username=#{username}")
