package com.gdeiassistant.gdeiassistant.Service.Wechat;

import com.gdeiassistant.gdeiassistant.Enum.Wechat.RequestTypeEnum;
...
import com.gdeiassistant.gdeiassistant.Pojo.ScheduleQuery.ScheduleQueryResult;
...
import com.gdeiassistant.gdeiassistant.Service.CardQuery.CardQueryService;
import com.gdeiassistant.gdeiassistant.Service.GradeQuery.GradeQueryService;
...
public class WechatService {
    ...
    public WechatBaseMessage HandleUserRequest(HttpServletRequest request, WechatBaseMessage wechatBaseMessage
        ...
        if (StringUtils.isNotBlank(username)) {
            ...
            return new WechatTextMessage(wechatBaseMessage, "你未绑定微信账号，请发送如下格式文本" +
                    "进行绑定：绑定账号-教务系统账号用户名-教务系统账号密码。例：绑定账号-gdeiassistant-123456");
        }
        return new WechatTextMessage(wechatBaseMessage, "你未绑定微信账号，请发送如下格式文本" +
                "进行绑定：绑定账号-教务系统账号用户名-教务系统账号密码。例：绑定账号-gdeiassistant-123456");
    }
    ...
    private WechatBaseMessage HandleAttachRequest(HttpServletRequest request, String wechatId
        ...
        return new WechatTextMessage(wechatBaseMessage, "发送的文本格式有误，请检查并重试！" +
                "绑定账号文本格式：绑定账号-教务系统账号用户名-教务系统账号密码。例：绑定账号-gdeiassistant-123456");
    }
}
