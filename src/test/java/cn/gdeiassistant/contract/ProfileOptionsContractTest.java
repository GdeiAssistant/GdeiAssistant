...
class ProfileOptionsContractTest {
    ...
    @Test
    void getProfileOptionsReturnsCodeOnlyDictionaryPayload() throws Exception {
        mockMvc.perform(get("/api/profile/options").header("Accept-Language", "en-US"))
                ...
                .andExpect(jsonPath("$.data.faculties[0].majors[0]").value("unselected"))
                .andExpect(jsonPath("$.data.faculties[11].majors[1]").value("software_engineering"))
